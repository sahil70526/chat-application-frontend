...
export const addNewProfileImage = async (file) => {
  try {
    const token = localStorage.getItem('userToken');
    const { data } = await axios.post(`${url}/api/users/add-profile-image`, { profile: file }, { headers: { "Content-Type": "multipart/form-data" } });
    return data;
  } catch (error) {
